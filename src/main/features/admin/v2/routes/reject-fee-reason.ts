...
export default express.Router()
	...
	.post(Paths.feeRejectReason.uri,
    validateFeeParams([
      { source: 'params', name: 'feeCode', field: 'feeCode', required: true },
      { source: 'params', name: 'version', field: 'feeVersion', required: true }
    ]),
    FormValidator.requestHandler(RejectFeeForm, RejectFeeForm.fromObject), (req: express.Request, res: express.Response) => {
  const form: Form<RejectFeeForm> = req.body

	  if (form.hasErrors()) {
	    Renderer.renderPage(form, res)
	  } else {
		  FeesClient.reasonForRejectFee(res.locals.user, req.params.feeCode, Number(req.params.feeVersion), form.model.toDto() as ReasonDto).then(
				() =>
				  res.render('admin/v2/views/fee-rejected-confirmation', {
				  feeCode: req.params.feeCode
			  })
		  ).catch(
			  (e: Error) => {
