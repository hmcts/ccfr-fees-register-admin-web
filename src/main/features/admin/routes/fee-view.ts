...
function renderView (categoryId: string, form: Form<FeeForm>, res: express.Response): void {
...
export default express.Router()
  .get(Paths.feeViewPage.uri, (req: express.Request, res: express.Response) => {
    const {feeCode, categoryId} = req.params

    FeesClient
      .retrieveFee(feeCode)
      .then((fee: Fee) => {
        renderView(categoryId, new Form(new FeeForm(fee.code, fee.type, fee.description, fee.amount / 100, fee.percentage)), res)
      })
  })
  .post(Paths.feeViewPage.uri, FormValidator.requestHandler(FeeForm, FeeForm.fromObject), (req: express.Request, res: express.Response, next: express.NextFunction) => {
    ...
    if (form.hasErrors()) {
      renderView(categoryId, form, res)
    } else {
      FeesClient
        .updateFee(new Fee(model.code, model.type, model.description, model.amount * 100, model.percentage))
        .then((fee: Fee) => {
          res.redirect(Paths.categoryViewPage.uri.replace(':categoryId', categoryId))
        })
    }
  })
