...
function () {
  ...
  CookieHandler.saveCookie = function (cookie, expiry, secure) {
    ...
    cookieString += _typeof(cookie.value) === 'object' ? JSON.stringify(cookie.value) : cookie.value;
...
function () {
  ...
  UserPreferences.prototype._loadPreferencesFromCookie = function () {
    ...
    if (_typeof(cookiePreferences) !== 'object') {
...
function () {
  ...
  CookieBannerHandler.prototype._clickEventHandler = function (event, name, confirmationClass, consent) {
    ...
    if (consent !== undefined) {
      var preferences_1 = this.userPreferencesHandler.getPreferences(); // If set to TRUE (consent all) or FALSE (reject all)
...
var isBoolean = function isBoolean(property) {
...
var isUndefined = function isUndefined(property) {
  return property === undefined;
};

var isArray = function isArray(property) {
  return Array.isArray(property) && property.length;
};

var isArrayOfType = function isArrayOfType(property, type) {
...
function () {
  function ConfigHandler() {
    this.configTypes = {
      ...
      cookieManifest: {
        ...
        cookies: function cookies(property) {
          return isArrayOfType(property, isString);
        }
      }
    };
  ...
  ConfigHandler.prototype.mergeConfigurations = function (providedConfig) {
    ...
    var finalConfig = _assign({}, ConfigHandler.defaultConfig);

    if (Object.keys(providedConfig).length) {
      finalConfig.userPreferences = _assign(_assign({}, ConfigHandler.defaultConfig.userPreferences), providedConfig.userPreferences);
      finalConfig.additionalOptions = _assign(_assign({}, ConfigHandler.defaultConfig.additionalOptions), providedConfig.additionalOptions);
      finalConfig.preferencesForm = _assign(_assign({}, ConfigHandler.defaultConfig.preferencesForm), providedConfig.preferencesForm);
      finalConfig.cookieBanner = _assign(_assign({}, ConfigHandler.defaultConfig.cookieBanner), providedConfig.cookieBanner);
...
var _cookieManager = _interopRequireDefault(require("@hmcts/cookie-manager"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { "default": obj }; }

_cookieManager["default"].on('UserPreferencesLoaded', function (preferences) {
...
_cookieManager["default"].on('UserPreferencesSaved', function (preferences) {
  var dataLayer = window.dataLayer || [];
...
_cookieManager["default"].on('PreferenceFormSubmitted', function () {
  var message = document.querySelector('.cookie-preference-success');
...
_cookieManager["default"].init(config);

},{"@hmcts/cookie-manager":1}]},{},[2]);
