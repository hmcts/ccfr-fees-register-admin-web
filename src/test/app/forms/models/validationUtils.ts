...
export function expectPropertyValidationError (errors: ValidationError[], property: string, message: string) {
  ...
  expect(violations).to.contain(new Violation(property, message), `Error '${message}' has not been triggered for property '${property}'. The following errors has been triggered instead: ` + JSON.stringify(violations))
}
